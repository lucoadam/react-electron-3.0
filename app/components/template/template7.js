export default function returnHtml(props) {
    return(`
<html lang="en">

<head>
  <meta charset="utf-8">
  <title>A4</title>

  <!-- Normalize or reset CSS with your favorite library -->
  <style type="text/css">
    html {
      line-height: 1.15;
      -ms-text-size-adjust: 100%;
      -webkit-text-size-adjust: 100%;
    }

    body {
      margin: 0;
    }

    table {
      width: 100%;
    }

    article,
    aside,
    footer,
    header,
    nav,
    section {
      display: block;
    }

    h1 {
      font-size: 2em;
      margin: 0.67em 0;
    }

    figcaption,
    figure,
    main {
      display: block;
    }

    figure {
      margin: 1em 40px;
    }

    hr {
      box-sizing: content-box;
      height: 0;
      overflow: visible;
    }

    pre {
      font-family: monospace, monospace;
      font-size: 1em;
    }

    a {
      background-color: transparent;
      -webkit-text-decoration-skip: objects;
    }

    abbr[title] {
      border-bottom: none;
      text-decoration: underline;
      text-decoration: underline dotted;
    }

    b,
    strong {
      font-weight: inherit;
    }

    b,
    strong {
      font-weight: bolder;
    }

    code,
    kbd,
    samp {
      font-family: monospace, monospace;
      font-size: 1em;
    }

    dfn {
      font-style: italic;
    }

    mark {
      background-color: #ff0;
      color: #000;
    }

    small {
      font-size: 80%;
    }

    sub,
    sup {
      font-size: 75%;
      line-height: 0;
      position: relative;
      vertical-align: baseline;
    }

    sub {
      bottom: -0.25em;
    }

    sup {
      top: -0.5em;
    }

    audio,
    video {
      display: inline-block;
    }

    audio:not([controls]) {
      display: none;
      height: 0;
    }

    img {
      border-style: none;
    }

    svg:not(:root) {
      overflow: hidden;
    }

    button,
    input,
    optgroup,
    select,
    textarea {
      font-family: sans-serif;
      font-size: 100%;
      line-height: 1.15;
      margin: 0;
    }

    button,
    input {
      overflow: visible;
    }

    button,
    select {
      text-transform: none;
    }

    [type="reset"],
    [type="submit"],
    button,
    html [type="button"] {
      -webkit-appearance: button;
    }

    [type="button"]::-moz-focus-inner,
    [type="reset"]::-moz-focus-inner,
    [type="submit"]::-moz-focus-inner,
    button::-moz-focus-inner {
      border-style: none;
      padding: 0;
    }

    [type="button"]:-moz-focusring,
    [type="reset"]:-moz-focusring,
    [type="submit"]:-moz-focusring,
    button:-moz-focusring {
      outline: 1px dotted ButtonText;
    }

    fieldset {
      padding: 0.35em 0.75em 0.625em;
    }

    legend {
      box-sizing: border-box;
      color: inherit;
      display: table;
      max-width: 100%;
      padding: 0;
      white-space: normal;
    }

    progress {
      display: inline-block;
      vertical-align: baseline;
    }

    textarea {
      overflow: auto;
    }

    [type="checkbox"],
    [type="radio"] {
      box-sizing: border-box;
      padding: 0;
    }

    [type="number"]::-webkit-inner-spin-button,
    [type="number"]::-webkit-outer-spin-button {
      height: auto;
    }

    [type="search"] {
      -webkit-appearance: textfield;
      outline-offset: -2px;
    }

    [type="search"]::-webkit-search-cancel-button,
    [type="search"]::-webkit-search-decoration {
      -webkit-appearance: none;
    }

    ::-webkit-file-upload-button {
      -webkit-appearance: button;
      font: inherit;
    }

    details,
    menu {
      display: block;
    }

    summary {
      display: list-item;
    }

    canvas {
      display: inline-block;
    }

    template {
      display: none;
    }

    [hidden] {
      display: none;
    }
  </style>

  <!-- Load paper.css for happy printing -->
  <style type="text/css">
    @page {
      margin: 0;
    }

    body {
      margin: 0;
    }

    .sheet {
      margin: 0;
      overflow: hidden;
      position: relative;
      box-sizing: border-box;
      page-break-after: always;
    }

    body.A3 .sheet {
      width: 297mm;
      height: 419mm;
    }

    body.A3.landscape .sheet {
      width: 420mm;
      height: 296mm;
    }

    body.A4 .sheet {
      width: 210mm;
      height: 296mm;
    }

    body.A4.landscape .sheet {
      width: 297mm;
      height: 209mm;
    }

    body.A5 .sheet {
      width: 148mm;
      height: 209mm;
    }

    body.A5.landscape .sheet {
      width: 210mm;
      height: 147mm;
    }

    body.letter .sheet {
      width: 216mm;
      height: 279mm;
    }

    body.letter.landscape .sheet {
      width: 280mm;
      height: 215mm;
    }

    body.legal .sheet {
      width: 216mm;
      height: 356mm;
    }

    body.legal.landscape .sheet {
      width: 357mm;
      height: 215mm;
    }

    .sheet.padding-10mm {
      padding: 10mm;
    }

    .sheet.padding-15mm {
      padding: 15mm;
    }

    .sheet.padding-20mm {
      padding: 20mm;
    }

    .sheet.padding-25mm {
      padding: 25mm;
    }

    @media screen {
      body {
        background: #e0e0e0;
      }

      .sheet {
        background: #fff;
        box-shadow: 0 0.5mm 2mm rgba(0, 0, 0, 0.3);
        margin: 5mm auto;
      }
    }

    @media print {
      body.A3.landscape {
        width: 420mm;
      }

      body.A3,
      body.A4.landscape {
        width: 297mm;
      }

      body.A4,
      body.A5.landscape {
        width: 210mm;
      }

      body.A5 {
        width: 148mm;
      }

      body.legal,
      body.letter {
        width: 216mm;
      }

      body.letter.landscape {
        width: 280mm;
      }

      body.legal.landscape {
        width: 357mm;
      }
    }

    * {
      font-family: Preeti, serif;
      font-size: 16pt;
    }
  </style>

  <!-- Set page size here: A5, A4 or A3 -->
  <!-- Set also "landscape" if you need -->
  <style>
    @page {
      size: A4;
    }
  </style>
</head>

<!-- Set "A5", "A4" or "A3" for class name -->
<!-- Set also "landscape" if you need -->

<body>
  <!-- Each sheet element should have the class "sheet" -->
  <!-- "padding-**mm" is optional: you can set 10, 15, 20 or 25 -->
  <div type="HEADER">
    <p class="western" align="CENTER" style="margin-bottom: 0in;line-height: 100%;">
      <font face="Preeti, serif">
        <font size="4" style="font-size: 15pt"><b>g]kfn ;/sf/</b></font>
      </font><img
        src="data:image/jpeg;base64,"
        name="Image1" align="LEFT" hspace="12" width="84" height="66" border="0" style="
  margin-left: 30px;
  margin-right: -99px;
">
    </p>

    <p class="western" align="CENTER" style="margin-bottom: 0in;line-height: 100%;margin-top: 8px;">
      <font face="Preeti, serif">
        <font size="4" style="font-size: 15pt"><b>e"ld Joj:yf ;xsf/L tyf ul/aL lgjf/0f dGqfno</b></font>
      </font>
      <font face="Preeti, serif">
        <font size="4" style="font-size: 15pt"></font>
      </font>
    </p>

    <p class="western" align="CENTER" style="margin-bottom: 0in;line-height: 100%;margin-top: 8px;margin-left: 10px;">
      <font face="Preeti, serif">
        <font size="4" style="font-size: 16pt"><b>e"ld Joj:yfkg tyf clen]v ljefu</b></font>
      </font>
    </p>
    <p class="western" align="CENTER" style="margin-left: 0.2in;margin-bottom: 0in;line-height: 100%;margin-top: 8px;">
      <font face="Preeti, serif">
        <font size="6" style="font-size: 15pt"><b>dfnkf]t sfof{no</b></font>

      </font>
    </p>
    <p class="western" align="CENTER" style="margin-left: 0.2in;margin-bottom: 0in;line-height: 100%;margin-top: 8px;">
      <font face="Preeti, serif">
        <font size="6" style="font-size: 15pt"><b> ;fFv',sf7df8f}</b></font>

      </font>
    </p>
    <p class="western" style="margin-left: -0.1in; margin-bottom: 0in; line-height: 100%">
      <font face="Preeti, serif">
        <font size="2" style="font-size: 10pt">k= ;+</font>
      </font>
      <font face="Preeti, serif">
        <font size="2" style="font-size: 10pt">
          M )&amp;^÷&amp;&amp;
        </font>
      </font><span style="
  margin-left: 37%;
">
        <font face="Preeti, serif" style="
                  text-align: center;
                  ">

        </font><span></span>
      </span>
    </p>

    <p class="western" style="margin-left: -0.1in;margin-bottom: 0in;line-height: 100%;margin-top: 8px;">
      <font face="Preeti, serif">
        <font size="2" style="font-size: 10pt">r =g+= </font>
      </font>
    </p>
  </div>
  <p class="western" style="margin-bottom: 0.11in;margin-top: 0.11in;"><br><br></p>
  <p class="western" style="margin-bottom: 0.14in">
    <font face="Preeti, serif">
      <font size="4" style="font-size: 16pt"><span>&gt;L gfkL sfof{no </span><span style="
float: right;
">
          ldlt @)&amp;^. .</span><br>;fFv',sf7df8f}</font>
    </font>
  </p>
  <p align="CENTER" style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">ljifo
        M gfd;f/L .</font>
    </font>
  </p>
  <p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">lga]bg btf{
        g++=============${props.dartaNumber}=================</font>
    </font>
  </p>
  ${props.showNibedakArray ? (`<p style="margin-bottom: 0.11in">
    <a name="_GoBack"></a>
    <font face="Preeti, serif">
      <font size="4">!=lga]bssf] lja/0f</font>
    </font>
  </p>
  <table width="612" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="89">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="77">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gffd,y/</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf g+=</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L ldlt</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L lhNnf</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affa'÷kltsf] gfd</font>
            </font>
          </p>
        </td>
        <td width="77"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affh]÷;;'/fsf] gfd</font>
            </font>
          </p>
        </td>
      </tr>

      ${props.nibedakArray.map(function (each) {
            return (` <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p><br>${each.nibedakNaam}</p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p><br>${each.nibedakThgan}</p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p><br>${each.nibedakdartanikaya}</p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p><br>${each.nibedakdartaMiti}</p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p><br>${each.nibedakdartaNum}</p>
        </td>
        <td width="77"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p><br>${each.nibedakkai}</p>
        </td>
      </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  ${props.showJaggaArray ? (`<p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">@=hUUffwgLsf]
        lja/0f</font>
    </font>
  </p>
  <table width="612" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="89">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="77">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gffd,y/</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf g+=</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L ldlt</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L lhNnf</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affa'÷kltsf] gfd</font>
            </font>
          </p>
        </td>
        <td width="77"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affh]÷;;'/fsf] gfd</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.jaggaArray.map(function (each) {
        return (` <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaNaamThar}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaNagariktaNumber}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaNagariktaJariMiti}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaNagariktaJariJilla}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaBabuPitakoNaam}<br>
          </p>
        </td>
        <td width="77"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaBajeSasuraNaam}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  ${props.showJaggabibaranArray ? (`<p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">#=</font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
        bflvn vf/]h</font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
        x'g] hUufsf] lja/0f</font>
    </font>
  </p>
  <table width="600" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="89">
      <col width="68">
      <col width="136">
      <col width="66">
      <col width="169">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">lhNnf</font>
            </font>
          </p>
        </td>
        <td width="68"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Ufflj;</font>
            </font>
          </p>
        </td>
        <td width="136"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Jf8f g++=÷l;6 g++=</font>
            </font>
          </p>
        </td>
        <td width="66"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">lsQf g++</font>
            </font>
          </p>
        </td>
        <td width="169"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">If]qkmn</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.jaggabibaranArray.map(function (each) {
        return (` <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggabibaranJilla}<br>
          </p>
        </td>
        <td width="68"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggabibaranGabisa}<br>
          </p>
        </td>
        <td width="136"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggabibaranWadaNumberSitNumber}<br>
          </p>
        </td>
        <td width="66"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggabibaranKitaNumber}<br>
          </p>
        </td>
        <td width="169"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggabibaranKsytrafal}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  ${props.showDakhilArray ? (`<p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">$=
      </font>
    </font>
    <font face="Preeti, serif">
      <font size="4">bflvn vf/]h e}
        sfod x'g] </font>
    </font>
    <font face="Preeti, serif">
      <font size="4"><b>AolQm÷
          ;+:yfsf</b></font>
    </font>
    <font face="Preeti, serif">
      <font size="4">] </font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
        laa/0f</font>
    </font>
  </p>
  <table width="623" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="64">
      <col width="72">
      <col width="72">
      <col width="72">
      <col width="78">
      <col width="83">
      <col width="82">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="64"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gffd,y/</font>
            </font>
          </p>
        </td>
        <td width="72"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf g+=</font>
            </font>
          </p>
        </td>
        <td width="72"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L ldlt</font>
            </font>
          </p>
        </td>
        <td width="72"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L lhNnf</font>
            </font>
          </p>
        </td>
        <td width="78"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affa'÷kltsf] gfd</font>
            </font>
          </p>
        </td>
        <td width="83"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affh]÷;;'/fsf] gfd</font>
            </font>
          </p>
        </td>
        <td width="82"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">hUUffwgL;Fusf] gftf</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.dakhilArray.map(function (each) {
        return (` <tr valign="TOP">
        <td width="64"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.dakhilNaamThar}<br>
          </p>
        </td>
        <td width="72"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.dakhilNagariktaNumber}<br>
          </p>
        </td>
        <td width="72"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.dakhilNagariktaJariMiti}<br>
          </p>
        </td>
        <td width="72"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.dakhilNagariktaJariJilla}<br>
          </p>
        </td>
        <td width="78"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.dakhilBabuPitakoNaam}<br>
          </p>
        </td>
        <td width="83"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.dakhilBajeSasuraNaam}<br>
          </p>
        </td>
        <td width="82"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.jaggaDhaniSangakoNata}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  ${props.showNibedak1Array ? (`<p style="margin-bottom: 0.11in">
  <a name="_GoBack"></a>
    <font face="Preeti, serif">
      <font size="4"><b>;+:yf
          eP -cfkm}F_</b></font>
    </font>
  </p>
  <table width="612" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="89">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="77">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">;+:yfsf] gffd </font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">7]ufgf+=</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">btf{ ePsf] lgsfo</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">btf{ ldlt </font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">btf{ g++</font>
            </font>
          </p>
        </td>
        <td width="77"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">s}lkmot</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.nibedak1Array.map(function (each) {
        return (` <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.nibedak1Naam}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.nibedak1Thgan}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.nibedak1dartanikaya}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.nibedak1dartaMiti}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.nibedak1dartaNum}<br>
          </p>
        </td>
        <td width="77"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.nibedak1kai}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  ${props.showSipharisArray ? (`<p style="margin-bottom: 0.11in">
  <br><br>
  </p>
  <p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">^=</font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
        a}Fs tyf ljQLo ;+:yfaf6 bflvn vf/]hsf] nflu cg'/f]w ug{]</font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
        lgsfo</font>
    </font>
  </p>
  <table width="612" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="249">
      <col width="76">
      <col width="88">
      <col width="141">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="249"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">lgsfosf] gfd</font>
            </font>
          </p>
        </td>
        <td width="76"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">lhNnf</font>
            </font>
          </p>
        </td>
        <td width="88"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Kfqsf] r=g++</font>
            </font>
          </p>
        </td>
        <td width="141"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">ldlt</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.sipharisArray.map(function (each) {
        return (` <tr valign="TOP">
        <td width="249"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          
          <p>${each.sipharisNaam}<br>
          </p>
        </td>
        <td width="76"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.sipharisJilla}<br>
          </p>
        </td>
        <td width="88"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.sipharisChaNumber}<br>
          </p>
        </td>
        <td width="141"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.sipharisMiti}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
     
  <p style="margin-bottom: 0.11in">
  <br><br>
  </p>
  <p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">&amp;=df]7
        /f]Ssfsf] lja/0f M ;DalGwt df]7 kmfF6af6
        =================${props.phatbatajaggako1}=====================hUufsf] ;|]:tf sfod 5 /f]Ssf
        b]lvPg egL n]vL cfPsf] .</font>
    </font>
  </p>
  <p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">*=tfd]nLsf]
        Aoxf]/f M  ${props.behora1}</font>
    </font>
  </p>
  <p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">(=;+nUg
        k|df0f</font>
    </font>
  </p>
  <ul>
    <li>
    ${props.memeArray.map(function (each) {
      return (` <p style="margin-bottom: 0.11in">
        <font face="Preeti, serif">
          <font size="4">=================${each.nagarpalika}===========================sf]
            r g++======${each.chaNumber}======ldlt========${each.Miti}==========sf] ;fydf </font>
        </font>
        <font face="Preeti, serif">
          <font size="4">bflvn
            vf/]hsf] nflu cg'/f]w e} cfPsf] kq;fy a}Fssf] lg0f{o,;fj{hlgs
            ;"rgf,d'r'Nsf nfufotsf sfuhftx? </font>
        </font>
      </p>
    </li>
    <li>
      <p style="margin-bottom: 0.11in">
        <font face="Preeti, serif">
          <font size="4">hUuf
            wgL btf{ k|df0f kq k|ltlnkL</font>
        </font>
      </p>
    </li>
    <li>
      <p style="margin-bottom: 0.11in">
        <font face="Preeti, serif">
          <font size="4">gful/stfsf]
            k|df0fkqsf] k|ltlnkL</font>
        </font>
      </p>
    </li>
    <li>
      <p style="margin-bottom: 0.11in">
        <font face="Preeti, serif">
          <font size="4">/=g+========${each.raNumber}===========ldlt=========${each.aMiti}===============sf]
            ;Ssn tfd]nL b[li6aGwsL lnvt</font>
        </font>
      </p>`);
    })}
    </li>
    <li>
      <p style="margin-bottom: 0.11in">
        <font face="Preeti, serif">
          <font size="4">dfnkf]t
            lt/]sf] /l;b</font>
        </font>
      </p>
    </li>
  </ul>
  <p align="JUSTIFY" style="margin-left: 0.5in; margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4">!)
        sfof{nosf]=lg0f{o M ldl;n ;+nUg k|df0f tyf lja/0fsf cfwf/df</font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
        a}Fs tyf ljQLo ;+:yfaf6 bflvn vf/]h</font>
    </font>
    <font face="Preeti, serif">
      <font size="4">
      </font>
    </font>
    <font face="Preeti, serif">
      <font size="4">b:t'/
        ?======${props.rakam}======= tyf k'FhLut nfes/ afkt
        ?============${props.rakam1}=================u/L hDdf
        ?==============${props.rakam2}==============sf] ;+nUg a}Fs ef}r/ a'emL b]xfosf] hUuf
        b]xfosf AolQmsf gfddf gfd;f/L x'g plrt b]vL lg0f{ofy{ k]z u/]sf] 5' .
      </font>
    </font>
  </p>
  ${props.showDaKhil1Array ? (`<p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4"><b>bflvn
          vf/]h e} hfg] hUufsf] lja/0f</b></font>
    </font>
  </p>
  <table width="618" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="89">
      <col width="68">
      <col width="136">
      <col width="66">
      <col width="187">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">lhNnf</font>
            </font>
          </p>
        </td>
        <td width="68"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Ufflj;</font>
            </font>
          </p>
        </td>
        <td width="136"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Jf8f g++=÷l;6 g++=</font>
            </font>
          </p>
        </td>
        <td width="66"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">lsQf g++</font>
            </font>
          </p>
        </td>
        <td width="187"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">If]qkmn</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.daKhil1Array.map(function (each) {
        return (` <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.daKhilJilla}<br>
          </p>
        </td>
        <td width="68"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.daKhilGabisa}<br>
          </p>
        </td>
        <td width="136"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.daKhilOda}<br>
          </p>
        </td>
        <td width="66"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.daKhilKirta}<br>
          </p>
        </td>
        <td width="187"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.daKhilChetrafhal}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  ${props.showLastArray ? (`<p style="margin-bottom: 0.11in">
    <font face="Preeti, serif">
      <font size="4"><b>bflvn
          vf/]h e} hfg] </b></font>
    </font>
    <font face="Preeti, serif">
      <font size="4"><b>hUufwgLsf]
          laa/0f</b></font>
    </font>
  </p>
  <table width="623" cellpadding="7" cellspacing="0">
    <colgroup>
      <col width="89">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="90">
      <col width="89">
    </colgroup>
    <tbody>
      <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gffd,y/</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf g+=</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L ldlt</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Gfful/stf hf/L lhNnf</font>
            </font>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affa'÷kltsf] gfd</font>
            </font>
          </p>
        </td>
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>
            <font face="Preeti, serif">
              <font size="4">Affh]÷;;'/fsf] gfd</font>
            </font>
          </p>
        </td>
      </tr>
      ${props.lastArray.map(function (each) {
        return (` <tr valign="TOP">
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.lastNaamThar}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.lastNagariktaNumber}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.lastNagariktaJariMiti}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.lastNagariktaJariJilla}<br>
          </p>
        </td>
        <td width="90"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.lastBabuPitakoNaam}<br>
          </p>
        </td>
        <td width="89"
          style="border: 1px solid #00000a; padding-top: 0in; padding-bottom: 0in; padding-left: 0.08in; padding-right: 0.08in">
          <p>${each.lastBajeSasuraNaam}<br>
          </p>
        </td>
        </tr>`);
      })}
    </tbody>
  </table>`) : ('')}
  <p style="margin-left: 0.5in; margin-bottom: 0.11in"><br><br>
  </p>



</body>

</html>
    `);
}